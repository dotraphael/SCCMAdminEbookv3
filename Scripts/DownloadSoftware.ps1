...
"AdkW10Download;https://go.microsoft.com/fwlink/p/?linkid=859206;adksetup.exe",
"SCCMCB;http://care.dlservice.microsoft.com/dl/download/C/A/0/CA0CAE64-358C-49EC-9F61-8FACFEDE7083/SC_Configmgr_SCEP_1702.exe;SC_Configmgr_CB.exe",
...
try {
	if (Test-Path "$($rootFolder)\source\SCCMCB\Extract") {
		...
		start-sleep 5
		Start-Process -FilePath ("c:\windows\system32\robocopy.exe") -ArgumentList ("C:\SC_Configmgr_SCEP_1702 $($rootFolder)\source\SCCMCB\Extract /s /e /copy:DAT /r:1 /w:1 /xj /xjd /xjf") -wait
		Remove-Item "C:\SC_Configmgr_SCEP_1702" -force -recurse
	}
} catch {
	Write-log -message "Error: $_" -severity 3
}
...
try {
	if (Test-Path "$($rootFolder)\source\SQL2016") {
		...
		Write-log -message "Extract SQL2016" -severity 1
