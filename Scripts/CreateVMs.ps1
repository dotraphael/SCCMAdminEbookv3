...
if ((get-vm -Name "CLASSROOM-SRV0003" -ErrorAction SilentlyContinue) -ne $null) {
	...
	try {
		if (-not (Test-Path -Path "$($rootFolder)\VHDX\srv0003.vhdx")) {
            ...
            Write-log -message "$($rootFolder)\VHDX\srv0003.vhdx already exist, ignoring it" -severity 2
        }
	} catch {
		Write-log -message "Error: $_" -severity 3
	}
	...
	try {
		if ($CreateVM) {
			...
			New-VM -Name "CLASSROOM-SRV0003" -VHDPath "$($rootFolder)\VHDX\srv0003.vhdx" -MemoryStartupBytes (1024 * 1024 * 1024) -SwitchName $InternalSwitch -Generation 1 -Path "$($rootFolder)\VHDX"
            ...
            Write-log -message "Creation of SRV0003 VM has been ignored because CreateVM variable is set to false" -severity 2
        }
	} catch {
		Write-log -message "Error: $_" -severity 3
	}
...
if ((get-vm -Name "CLASSROOM-WKS0003" -ErrorAction SilentlyContinue) -ne $null) {
	...
	try {
		if (-not (Test-Path -Path "$($rootFolder)\VHDX\wks0003.vhdx")) {
            ...
            Write-log -message "$($rootFolder)\VHDX\wks0003.vhdx already exist, ignoring it" -severity 2
        }
	} catch {
		Write-log -message "Error: $_" -severity 3
	}
	...
	try {
		if ($CreateVM) {
			...
			New-VM -Name "CLASSROOM-WKS0003" -VHDPath "$($rootFolder)\VHDX\WKS0003.vhdx" -MemoryStartupBytes (1024 * 1024 * 1024) -SwitchName $InternalSwitch -Generation 1 -Path "$($rootFolder)\VHDX"
            ...
            Write-log -message "Creation of WKS0003 VM has been ignored because CreateVM variable is set to false" -severity 2
        }
	} catch {
		Write-log -message "Error: $_" -severity 3
	}
